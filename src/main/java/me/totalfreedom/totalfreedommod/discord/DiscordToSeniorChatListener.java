...
public class DiscordToSeniorChatListener extends ListenerAdapter
{
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (event.getMember() != null && !chat_channel_id.isEmpty() && event.getChannel().getId().equals(chat_channel_id) && !event.getAuthor().getId().equals(event.getJDA().getSelfUser().getId())) {
            ...
            StringBuilder logmessage = new StringBuilder(ChatColor.DARK_GRAY + "[" + ChatColor.DARK_AQUA + "Discord SeniorChat" + ChatColor.DARK_GRAY + "] " + ChatColor.RESET);
    ...
    public Displayable getDisplay(Member member) {
        ...
        // Co-Owners
        else if (member.getRoles().contains(server.getRoleById(ConfigEntry.DISCORD_CO_OWNER_ROLE_ID.getString()))) {
